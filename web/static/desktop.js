$(document).ready(function(){
    ...
    $.get("/balances", function(data){
      ...
      if (data.addresses) {
        for (let i = 0; i < data.addresses.length; i++) {
          options = options + 
          "<option value=\"" + data.addresses[i].Address + "\">" + 
              data.addresses[i].Nickname + " (address)</option>";
        }
      }
      if (data.pubkeys) {
        for (let i = 0; i < data.pubkeys.length; i++) {
          options = options + 
          "<option value=\"" + data.pubkeys[i].Pubkey + "\">" + 
              data.pubkeys[i].Nickname + " (pubkey)</option>";
        }
      }
  ...
  function getAddressDetails() {
    ...
    $.post("/balance", JSON.stringify({Identifier: value})).done(function(data) {
      resp = data.reqInfo
      address = ""
  ...
  $("#add").click(function(){
      ...
      $.post("/watch", JSON.stringify({Identifier: identifier, Nickname: nickname})).always(function(data) {
        message = "Success"
